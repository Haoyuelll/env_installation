...
def build_chain_from_urdf(data, device="cpu"):
    """
    ...
    return chain.Chain(root_frame, device=device)


def build_serial_chain_from_urdf(data, end_link_name, root_link_name="", device="cpu"):
    """
    ...
    return chain.SerialChain(
        urdf_chain, end_link_name, root_frame_name=root_link_name, device=device
    )
